...
public class MyHashMap<K, V> implements Map61B<K, V> {
    ...
    public MyHashMap(int initialSize) {
        ...
        DEFAULT_INITIAL_SIZE = initialSize;
    }
    ...
    public void put(K key, V value) {
        ...
        if ((double) size / buckets.length > maxLoadFactor) {
            Collection<Node>[] newTable = createTable(2 * buckets.length);
            for (Collection<Node> bucket : buckets) {
                for (Node node : bucket) {
                    newTable[Math.floorMod(node.key.hashCode(), 2 * buckets.length)].add(node);
                }
            }
            buckets = newTable;
        }
    ...
    public V remove(K key, V value) {
        if (containsKey(key) && Objects.requireNonNull(getNode(key)).value.equals(value)) {
            return remove(key);
        }
        return null;
    }
